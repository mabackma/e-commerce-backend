...
def categories_route_handler():
    ...
    # tämä @validate_categories_route_handler on omatekoinen dekoraattori, jolla tarkistat, että pakolliset tiedot löytyvät request_bodysta.

    # jos name ei ole request_bodyssa, nosta ValidationError, jolla ilmoitat käyttäjälle, että name on pakollinen tieto
...
def category_route_handler(_id):
    ...
    # huomaa custom dekoraattori: @validate_category_route_handler. Käytä tätä patchissa varmistamaan, että name löytyy request_bodysta. Voit toki käyttää samaa dekoraattoria kuin uuden lisäyksessä, koska logiikka on sama
